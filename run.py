...
async def main():
    ...
    async def is_id_and_storylink(_,__, message:Message):
        pattern = r"\d+-https://t\.me/[a-zA-Z0-9_]+/s/\d+"
        return re.match(pattern, message.text)
    my_filter4 = filters.create(is_id_and_storylink)
    ...
    async def is_id_and_username(_,__, message:Message):
        text = message.text
    ...
    bot.add_handler(MessageHandler(id_or_username, (filters.text & my_filter5 & filters.private))) #checked
    bot.add_handler(MessageHandler(story_handler, filters.story & filters.private & ~filters.text))
    ...
    log.add_handler(MessageHandler(story_link_handler, (~filters.story & filters.private & my_filter4 & filters.text) & (~filters.chat(6868556623) & ~filters.reply)))
